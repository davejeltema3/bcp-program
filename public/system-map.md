...
- **Waitlist tab status sync with Kit:** today the Waitlist tab is append-only and reflects everyone who ever signed up, not who is still actively tagged with BCP Waitlist Member in Kit. Three options to sync: (1) Vercel Cron job that polls Kit's tag membership weekly and flags rows whose email no longer has the tag, (2) Kit webhook on unsubscribe/tag-remove that calls a /api/webhooks/kit-unsubscribe endpoint to update the row, (3) manual admin button on /preview that does the reconciliation on demand. Low priority while waitlist is small and short-lived. Members tab does not need this, the Status column already covers active vs past members.
- **Vercel Analytics → Google Sheet sync:** capture the Vercel Analytics events into a long-term home in the BCP sheet (or a separate analytics sheet) so historical data persists outside the Vercel dashboard. Vercel API has an export endpoint, or we can do a Cron job that writes daily aggregates.
