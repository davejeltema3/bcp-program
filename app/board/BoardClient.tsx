...
export default function BoardClient({ initial }: { initial: BoardData }) {
  ...
  const unseen = useMemo(
    () => board.notifications.filter((n) => n.ts > seen),
    [board.notifications, seen],
  );
  ...
  const markSeen = useCallback(async () => {
    ...
    try {
      ...
      /* offline is fine, the dot just re-shows next load */
    }
  }, []);

  // Tab red-dot: favicon + title, exactly like unread mail.
  useEffect(() => {
  ...
  // Clear the dot when you actually look at the tab.
  useEffect(() => {
    function onVis() {
      if (document.visibilityState === 'visible' && unseen.length > 0) {
        const t = setTimeout(() => markSeen(), 900);
  ...
  const blueLink: CSSProperties = { color: 'var(--bc-blue-300, #5b9cff)', textDecoration: 'none' };

  return (
    <main className={privacy ? 'bc-private' : undefined} style={{ background: 'var(--bc-ink-900, #0b1220)', minHeight: '100vh' }}>
      <style>{`
      ...
      <div style={wrap}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          <div>
            ...
            <div style={{ color: 'var(--bc-text-400, #6b7591)', fontSize: 13, marginTop: 2 }}>
              Thursday, July 16, 2026 {board.updated ? `· synced ${board.updated}` : ''}
            </div>
          </div>
          <button
            ...
            title="Blur sensitive content for streaming or screenshots. Hover any line to reveal it."
          >
        ...
        {board.notifications.length > 0 && (
          <section style={{ ...card, borderColor: unseen.length ? 'var(--bc-blue-400, #3a85ff)' : 'var(--bc-ink-600, #2a3654)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              {unseen.length > 0 && (
                <button onClick={markSeen} style={{ ...blueLink, background: 'none', border: 'none', cursor: 'pointer', fontSize: 12 }}>
                  Mark seen
                </button>
              )}
            </div>
        ...
        {board.goals.length > 0 && (
          <section style={card}>
            ...
            {board.goals.map((g, i) => (
              <div key={g.id} style={{ display: 'flex', gap: 10, padding: '6px 0', fontSize: 15, color: g.checked ? 'var(--bc-text-500, #4a546d)' : undefined }}>
                <span style={{ color: g.checked ? 'var(--bc-text-500, #4a546d)' : 'var(--bc-blue-300, #5b9cff)', fontWeight: 700 }}>{i + 1}</span>
                <span className="bc-sensitive" style={{ textDecoration: g.checked ? 'line-through' : 'none' }}>{g.text}</span>
              </div>
            ))}
          </section>
        )}
        ...
        {board.needsYou.length > 0 && (
          <section style={{ ...card, borderColor: 'var(--bc-ink-500, #3d4a6b)' }}>
            ...
            {board.needsYou.map((it) =>
              it.kind === 'item' ? (
                <ItemRow key={it.id} item={it} onToggle={toggle} sensitive />
              ) : (
        ...
        <div style={{ color: 'var(--bc-text-500, #4a546d)', fontSize: 12, marginTop: 18 }}>
          Check things off here or just tell Claude in a chat. Both write to the same board.
        </div>
      </div>
    </main>
  );
